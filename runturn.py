...
from app.python_testing.energize_functions import *
...
def main():
  ...
  nCities = 50
  xy = numpy.random.uniform(0,1000,[nCities,2])
  
  graph = networkx.Graph()
  for idx in range(len(xy)):
    graph.add_node( (xy[idx,0],xy[idx,1]), x=xy[idx,0], y=xy[idx,1])
  rng(graph)

  pop = 10**numpy.random.uniform(numpy.log10(8e2),numpy.log10(8e6),[nCities])
