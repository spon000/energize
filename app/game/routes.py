...
def player_generators():
  ...
  player_generators = dict()
  generator_schema = GeneratorSchema(many=True)

  for facility in facilities:
    facility_generators = list(facility.generators)
    serialized_facility_generators = generator_schema.dump(facility_generators)

    for gen in serialized_facility_generators[0]:
      generator_type = GeneratorType.query.filter_by(id=gen['generator_type']).first()
      gen['build_time'] = generator_type.build_time
      gen['lifespan'] = generator_type.lifespan
      gen['fixed_cost_build'] = generator_type.fixed_cost_build
      gen['nameplate_capacity'] = generator_type.nameplate_capacity

      gen['generator_age'] = get_age(game, gen['start_prod_date'], True)

    player_generators.update({str(facility.id): serialized_facility_generators[0]})
  ...
  return jsonify({'player_generators': player_generators})
